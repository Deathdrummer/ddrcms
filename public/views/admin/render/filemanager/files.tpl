	...
	{% for file in files %}
		<div class="{% if to_client %}clientfilemanager__file{% else %}filmanager__file{% endif %}" dirfile="{{file.src}}" namefile="{{file.name}}" extfile="{{file.ext}}">
			<div class="image noselect">
				...
				{% else %}
					<img src="{{images('filetypes/'~file.src|filename(2)~'.png', 'filetypes/untiped.png')}}" alt="{{file.name|filename(1)}} | файл {{file.name|filename(2)}}" title="{{file.name|filename(1)}} | файл {{file.name|filename(2)}}">
				{% endif %}
			</div>
			<span>{{file.name}}</span>
		</div>
